...
var (
	...
	// will be returned from the return statement of adding the index. This is used to bind the runtime. The way to get id to Risk is
	...
	//independently. Global network configuration
...
func GetDefaultYakitBaseDir() string {
	// . This detects the default database
...
func InitializeCVEDatabase() (*gorm.DB, error) {
	...
	// issue #725 This step must be done before adding the index, otherwise
	// . If there is no table, delete the file generated by open
	...
	initOnce.Do(func() {
		...
		if runtime.GOOS == "darwin" {
			if permutil.IsFileUnreadAndUnWritable(projectDatabaseName) {
				log.Info("encountered a permission problem when opening the project database, and tried to fix the database permission error independently.")
				if err := permutil.DarwinSudo(
					"chmod +rw "+strconv.Quote(projectDatabaseName),
					permutil.WithVerbose("and fixes the Yakit project database permissions."),
				); err != nil {
			...
			/*. Repair the profile db.*/
			if permutil.IsFileUnreadAndUnWritable(profileDatabaseName) {
				log.Info(". If you encounter permission problems when opening the user plug-in database, try to repair")
				if err := permutil.DarwinSudo(
					"chmod +rw "+strconv.Quote(profileDatabaseName),
					permutil.WithVerbose(". Repair the Yakit user database permissions"),
				); err != nil {
		...
		/* . First create the plug-in database */
		...
		/* created the project database again. */
