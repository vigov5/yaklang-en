// Copyright 2015 The Go Authors. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

package pkcs12

import (
	"encoding/base64"
	"fmt"
	"io/ioutil"
	"os"
	"testing"

	"github.com/yaklang/yaklang/common/gmsm/sm2"
	"github.com/yaklang/yaklang/common/gmsm/x509"
)

func Test_P12Encrypt(t *testing.T) {
	str := ""
	decodeBytes, err := base64.StdEncoding.DecodeString(str)
	x, err := x509.ParseCertificate(decodeBytes)
	priv, err := sm2.GenerateKey(nil) // Generate key pair
	if err != nil {
		fmt.Print(err)
		return
	}
	privPem, err := x509.WritePrivateKeyToPem(priv, nil) // Generate key file
	if err != nil {
		t.Fatal(err)
	}
	err = ioutil.WriteFile("priv.pem", privPem, os.FileMode(0644))
	if err != nil {
		t.Fatal(err)
	}
	privKey, err := x509.ReadPrivateKeyFromPem(privPem, nil) // Read key
	if err != nil {
		t.Fatal(err)
	}
	SM2P12Encrypt(x, "123", privKey, "test.p12") //Generate p12 certificate with password based on certificate and private key

}
func Test_P12Dncrypt(t *testing.T) {
	certificate, priv, err := SM2P12Decrypt("test.p12", "123") //Read P12 certificate based on password
	if err != nil {
		t.Fatal(err)
	}
	privPem, _ := ioutil.ReadFile("priv.pem")
	privatekey, err := x509.ReadPrivateKeyFromPem(privPem, nil)
	if err != nil {
		t.Fatal(err)
	}
	fmt.Println(certificate.Issuer)
	fmt.Println(privatekey.D.Cmp(priv.D) == 0)
	fmt.Println(priv.IsOnCurve(priv.X, priv.Y))
}
