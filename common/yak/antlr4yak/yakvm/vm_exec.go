...
func (v *Frame) execExWithContinueOption(isContinue bool) {
	...
	//Exit code // -1 represents an exception, 0 It means that the code is executed to the end, 1 means to exit
	...
	defer func() {
		...
		// indebuggerEval does not handle panic and throws it directly
		if !v.vm.GetConfig().GetStopRecover() && !v.indebuggerEval {
			if ierr = recover(); ierr != nil {
				if vmPanic, ok = ierr.(*VMPanic); !ok {
					// The panic generated by go requires a layer of VMPanic
		...
		// If it isContinue, it proves that it is executed in tryBlock. There is no need to handle exceptions and callbacks.
		if !isContinue && v.vm.debugMode {
			if v.indebuggerEval {
				if ierr != nil {
					// Execute the code in the debugger, and panic directly The debugger captures
					panic(ierr)
				}
				return
			}
			...
			} else if v.parent == nil { // . The program exits
				if debugger != nil {
...
func (v *Frame) _execCode(c *Code, debug bool) {
	...
	case OpScopeEnd:
		v.ExitScope()
	case OpAsyncCall:
		// . The bottom layer calls goroutine to operate
		wavy := false
		if c.Op1 != nil {
			wavy = c.Op1.Bool()
		}
	...
	case OpAssign:
		...
		case NASL:
			...
			if arg1.IsLeftValueRef() {
				...
				if arg2.IsYakFunction() { // nasl functions are saved to the global
					table[arg1.SymbolId] = arg2
					v.push(arg2)
					return
				} else {
	...
	case OpInNext:
		...
		// . To process the sliceIterator, the first index should discard
		if typ == SliceIteratorType && len(data) > 1 {
			data = data[1:]
		}
		// Unpack the sliceIterator
		value := data[0]
		firstReflectValue := reflect.ValueOf(value)
		firstReflectValueKind := firstReflectValue.Kind()
		// only when the number of lvalues is greater than 1. Package
	...
	case OpPush:
		...
		if c.Op1.IsStringOrBytes() {
			...
			case 'x':
				//directly. The f prefix is used to generate a string slice
				value, err := mutate.FuzzTagExec(strValue)
				if err != nil {
					v.push(NewStringSliceValue([]string{}))
					log.Error(err)
					//. Do not panic when parsing the fuzztag error to prevent the language engine exception caused by the fuzztag parsing failure.
		...
		return
	case OpPushLeftRef:
		// Find the corresponding value through the symbol
		id := c.Unary
	...
	case OpPushRef:
		...
		case NASL:
			...
			if !ok {
				...
				val1 := *val // Function parameters and formal parameter names in nasl are bound, here you need to copy a
				val = &val1
			}
			if !ok {
	...
	case OpPushId:
		// . It is not a symbol for assignment. Generally,
		// GlobalVariables can handle this problem very well
		name := c.Op1.String()
	...
	case OpTypeCast:
		...
		//if typeN == 2 { //normally and performs compound type judgment
		//	//If the conversion is unsuccessful, continue to try slice conversion
		...
		//	}
		//	v.push(resVal)
		//} else { //Basic type judgment

		// undefined processing and return a zero value
		if val.IsUndefined() {
		...
		//If the basic type is not successfully parsed, continue to try to parse the composite type
		...
		//is not successful, you can only try automatic conversion. If the conversion fails, panic the
	...
	case OpPlusPlus, OpMinusMinus:
		...
		scope := v.CurrentScope()
		if op.IsValueList() { // The lvalue is processed by iterablecall. Situation
	...
	case OpAnd, OpAndNot, OpOr, OpXor, OpShl, OpShr, OpAdd, OpSub, OpMul, OpDiv, OpMod,
		...
		/* is a general binary expression: the language itself does not have any computing power */
		args := v.popArgN(2)
	...
	case OpPlusEq, OpMinusEq, OpMulEq, OpDivEq, OpModEq, OpAndEq, OpAndNotEq, OpOrEq, OpXorEq, OpShlEq, OpShrEq:
		...
		scope := v.CurrentScope()
		if op1.IsValueList() { // The lvalue is processed by iterablecall. Situation
	...
	case OpCall:
		...
		case NASL:
			...
			//External function manually calls
		...
		case YAK:
			fallthrough
		default:
			// function call. This is very simple. Take N parameters from unary, put them into the call stack,
			wavy := false
			if c.Op1 != nil {
				wavy = c.Op1.Bool()
			}
	...
	case OpNewMap:
		...
		case LUA:
			...
			if c.Op1 == undefined {
				...
				} else { // The variable parameter receives an empty parameter, that is, no parameter is passed
				...
				if variadicPresent {
					...
					if len(values) == 0 { // variable parameters are empty map, that is, they are not passed. Then it should behave as nil when used to create a map.
						values[0] = undefined
					}
				}
	...
	case OpDefer:
		// op defer should be called during the overall execution to perform
	...
	case OpMemberCall:
		/*
			memberCall. Finally, it will generate Send a Value to the stack
				1. Generally speaking, this operation should not only extract its value, but also record the value path , what is the field name?
		*/
		switch v.vm.GetConfig().vmMode {
		case LUA:
			...
			case reflect.Map:
				...
				// map If the key cannot be found,
		...
		default:
			...
			// Get the go built-in member method
			...
			switch callerTypeKind {
			case reflect.Map:
				// go The built-in member method
				if findGolangBuiltinMemberMethod() {
					return
				}

				// First try to get the members of the map
				...
				// Finally get the built-in method
				if caller.GetExtraInfo("global") == nil { // If it is an ordinary variable, try to get the built-in method
					method, ok := mapBuildinMethod[memberName]
					if ok {
						ret := method.HandlerFactory(v, caller)
						literal := fmt.Sprintf("map.%s", memberName)
			...
			case reflect.Ptr, reflect.Struct:
				...
				// There may be a method to get the structure pointer
